...
void Flatulan::move()
{
    ...
    {
        temp = m_row;
        if (temp > 1 && temp <= m_city->rows() && m_city->player()->col() != m_col)  //not at the invlaid border, plus not encounter the player
            m_row--;
        else if (temp > 1 && temp <= m_city->rows() && m_city->player()->row() != temp - 1)
            m_row--;
        break;
    }
    case DOWN:
    {
        temp = m_row;
        if (temp >= 1 && temp < m_city->rows() && m_city->player()->col() != m_col)
            m_row++;
        else if (temp >= 1 && temp < m_city->rows() && m_city->player()->row() != temp + 1)
            m_row++;
        break;
    ...
    {
        temp = m_col;
        if (temp > 1 && temp <= m_city->cols() && m_city->player()->row() != m_row)
            m_col--;
        else if (temp > 1 && temp <= m_city->cols() && m_city->player()->col() != temp - 1)
            m_col--;
        break;
    }
    case RIGHT:
...
{
    m_age++;
    int temp1 = m_row, temp2 = m_col;
    if (m_city->determineNewPosition(temp1, temp2, dir) && m_city->nFlatulansAt(temp1, temp2) == 0)
    {
        switch (dir)       //not exceed the border, and not encounter the flatulan
        {
        case UP:
            m_row--;
            break;
...
{
    switch (dir)    //don't move the player if it's at the correponding border
    {
    case UP:
    {
        if (r == 1)
...
void City::display() const
{
    ...
    for (int i = 0; i < m_nFlatulans; i++)
    {
        if (grid[m_flatulans[i]->row()-1][m_flatulans[i]->col()-1] == '.')
            grid[m_flatulans[i]->row()-1][m_flatulans[i]->col()-1] = 'F';
        else if (grid[m_flatulans[i]->row()-1][m_flatulans[i]->col()-1] == 'F')
            grid[m_flatulans[i]->row()-1][m_flatulans[i]->col()-1] = '2';
        else if (grid[m_flatulans[i]->row()-1][m_flatulans[i]->col()-1] != '9')
            grid[m_flatulans[i]->row()-1][m_flatulans[i]->col()-1] ++;
    }
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    for (int i = 0; i < m_nFlatulans; i++)
    {
        if (m_player->row() <= m_flatulans[i]->row() + 1 && m_player->row() >= m_flatulans[i]->row() - 1 && m_player->col() <= m_flatulans[i]->col() + 1 && m_player->col() >= m_flatulans[i]->col() - 1)
        {
            if (m_flatulans[i]->possiblyGetConverted())
            {
                delete m_flatulans[i];      //if it's destroyed successfully, free the memory
                for (int k = i; k < m_nFlatulans - 1; k++)     //move all the memebers in m_flatulans one member forward
                    m_flatulans[k] = m_flatulans[k + 1];
                m_flatulans[m_nFlatulans] = nullptr;          //after moving, point the last pointer to null
                delete m_flatulans[m_nFlatulans];     //delete the last pointer
                m_nFlatulans--;       //decrease m_nFlatulans by one, since one flatulan is destroyed
                i--;
            }
        }
    }
...
void City::moveFlatulans()
{
    ...
    {
        m_flatulans[k]->move();
        if (isPlayerAt(m_flatulans[k]->row()-1, m_flatulans[k]->col()) || isPlayerAt(m_flatulans[k]->row() + 1, m_flatulans[k]->col())
            || isPlayerAt(m_flatulans[k]->row(), m_flatulans[k]->col() - 1) || isPlayerAt(m_flatulans[k]->row(), m_flatulans[k]->col() + 1))
        {
            m_player->getGassed();    //if the player is orthogonal to the flatulan, get gassed.
        }
    }
}
